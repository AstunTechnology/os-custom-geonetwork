            ...
            <div class="w-2/3">
              <xsl:call-template name="html-breadcrumb">
                <xsl:with-param name="breadcrumb">
                  ...
                  <a href="{$baseUrl}/collections/main" class="text-blue-600 underline"><xsl:value-of select="$mainCollectionName"/></a>
                </xsl:with-param>
              </xsl:call-template>
            </div>
            <div class="w-1/3">
          ...
          <div class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
            <xsl:for-each select="$collections">
              ...
              <xsl:call-template name="render-collection">
                ...
                <xsl:with-param name="url" select="concat($baseUrl, '/collections/', uuid)"/>
              </xsl:call-template>
            </xsl:for-each>
          </div>
        </div>
